# RainBird SIP Protocol — Reference

Source: [pyrainbird — SIP_PROTOCOL_COMMON.md](https://github.com/allenporter/pyrainbird/blob/main/docs/SIP_POROTOCL_COMMON.md)
by [@allenporter](https://github.com/allenporter), MIT license. See [`../CREDITS`](../CREDITS).

> Based on analysis of RainBird controller behavior and companion app v2.17.14

## Verification against this project

Cross-checked against [`../rainbird-sip-commands.js`](../rainbird-sip-commands.js) (2026-07-12). All command
codes, response codes, and response field positions/lengths already implemented in this project match this
spec exactly — including the full `CombinedControllerStateResponse` (`CC`) field layout used by
[`rainbird-state.js`](../rainbird-state.js) and [`rainbird-info.js`](../rainbird-info.js).

Notes on differences:
- `CurrentQueueRequest` (`3B`/`BB`) exists in this project's command table (marked "not implemented yet")
  but isn't documented in the upstream spec below.
- `ControllerFirmwareVersionResponse` (`8B`) has a full field schema (`major`/`minor`/`patch`) in this
  project; the upstream spec only lists the request/response codes without a field breakdown.
- `RetrieveScheduleResponse` (`A0`) and `CurrentQueueResponse` (`BB`) in this project's command table use a
  `decoder` field instead of position/length fields — schedule/queue decoding isn't implemented yet
  (`RetrieveScheduleRequest` is called out as untested in [`node-rainbird.js`](../node-rainbird.js)).

Everything this project actually sends/parses today (see the public methods on `RainBirdClass` in
`node-rainbird.js`) matches this reference, so it's a reliable source for extending support to
...
This matches `processResponse()` in [`../node-rainbird.js`](../node-rainbird.js).
...
This project does not currently implement this auto-follow behavior.
